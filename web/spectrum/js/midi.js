...
function midiNoteOn(noteNumber,midiChannel)
{
...
function midiNoteOff(noteNumber,midiChannel)
{
...
function midiCC(midiChannel,ccNumber,ccValue)
{
	...
	console.log('midiCC()',noteNumber,midiChannel);
	
	var message = [0x90+midiChannel, ccNumber, ccValue];
	var output = midiAccess.outputs.get(_portId);
	output.send( message );
}
